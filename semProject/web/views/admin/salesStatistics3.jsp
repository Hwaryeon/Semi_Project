...
<div id="container">
   <ul class="nav nav-tabs">
				...
				<li role="presentation" style="font-size: 14px;"><a href="<%= request.getContextPath() %>/salesSt.adm?type=t2">&nbsp;&nbsp;80%이상 성공 상품&nbsp;&nbsp;</a></li>
...
<%-- 페이지처리 --%>
		...
		<div class="pageArea" id="monthPaging" align="center">
			<a onclick="location.href='<%=request.getContextPath() %>/salesSt.adm?term=month&type=t2&currentPage=1'" class="link_fst">
			<span class="fa fa-angle-double-left" aria-hidden="true"><<</span></a>&#160;
			<% if (currentPage <= 1) { %>
				<a disabled class="link_prev"><</a>&#160;
			<% } else { %>
				<a onclick="location.href='<%=request.getContextPath() %>/salesSt.adm?term=month&type=t2&currentPage=<%=currentPage -1 %>'" class="link_prev"><</a>&#160;
			<% } %>
			...
			<% for(int p = startPage;p<= endPage;p++) { 
					if(p==currentPage) { %>
						<a disabled class="link_page" style="background:lightgray;"><%= p %></a>
			<% 		} else { %>
						<a onclick="location.href='<%=request.getContextPath()%>/salesSt.adm?term=month&type=t2&currentPage=<%=p %>'" class="link_page"><%= p %></a>
			<%  	} %>
			<% } %>
			...
			<% if(currentPage >= maxPage) { %>
				&#160;<a disabled class="link_next">></a>&#160;
			<% } else { %>
				&#160;<a onclick="location.href='<%=request.getContextPath()%>/salesSt.adm?term=month&type=t2&currentPage=<%=currentPage + 1%>'" class="link_next">></a>&#160;
			<% } %>
			<a onclick="location.href='<%=request.getContextPath()%>/salesSt.adm?term=month&type=t2&currentPage=<%=maxPage%>'" class="link_lst">>></a>
		</div>
...
$(function(){
	...
	$('select').change(function(){
	    if($("#year").is(':selected')) {
	    	...
	    	location.href = "<%= request.getContextPath() %>/salesSt.adm?term=month&type=t2&currentPage=1";
	    }else{
